...
import { rouletteABI, rouletteContractAddress, tokenABI, tokenContractAddress } from './contractDetails'
import { publicPolygonClient, walletPolygonClient, publicMantleSepoliaClient, walletMantleSepoliaClient } from './ViemClient';
...
const betResultListener = useMemo(() => {
	...
	const lockBet = async () => {
		...
		const approvalResponse = await walletMantleSepoliaClient.writeContract(approvalRequest.request);
...
const placeBet = (e, type, ind = 0, newVal = bet, revert = false) => {
	...
	const handleWithdrawWinnings = async (e) => {
		...
		try {
			...
			const withdrawSimulation = await publicMantleSepoliaClient.simulateContract({
				address: rouletteContractAddress,
				abi: rouletteABI,
			...
			const withdrawResponse = await walletMantleSepoliaClient.writeContract(withdrawSimulation.request);
	...
	useEffect(() => {
		const checkNetwork = async () => {
			...
			// load balance
			await publicMantleSepoliaClient.getBalance({ address: primaryWallet?.address }).then((res) => {
